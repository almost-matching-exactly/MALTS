...
insane.insaneLearning(X[:int(0.25*len(Y)),:],Y[:int(0.25*len(Y))],T[:int(0.25*len(Y))])
t_hat_insane = insane.CATE(X[int(0.25*len(Y)):,:])
...
dcate_cobyla_diag, t_hat_cobyla_diag = adknn_cobyla_diag.CATE(X[int(0.25*len(Y)):,:],X[:int(1.*len(Y)),:],Y[:int(1.*len(Y))],T[:int(1.0*len(Y))],fLC=fLCcobyla_diag)
...
for i in range(0,len(t_hat_cobyla_diag)):
    ...
    diamexp.append(np.exp(-0.4*diameterMatchGroup(dcate_cobyla_diag[i],LstarC_cobyla_diag)))
ATE_cobyla = np.sum( np.array(diamexp)*t_hat_cobyla_diag )/np.sum(np.array(diamexp))

def prune_bad_matches(t_hat,diameter,threshold=0.001):
    t_hat_prune = []
...
t_hat_cobyla_prune = prune_bad_matches(t_hat_cobyla_diag,diam,0.00001)
...
err = np.array(err)
svr = svm.SVR()
diam11 = np.reshape(np.array(diam),(-1, 1))
svr = svr.fit(diam11,t_hat_cobyla_diag)
...
plot(xspace,yspace,c='black',linestyle="dashed", linewidth=3.0)
xlim((-0.00001,0.00019))
#plot(diam2, func(diam2, popt[0],popt[1],popt[2]), color="black", linestyle="dashed", linewidth=3.0)
xlabel('1/Diameter of Matched group')
ylabel('CATE')
tight_layout()
fig.savefig('Figures/Lalonde_distancemetriclearning_MALTS_CATE_Diam_.jpg')
...
#
#BARTS
Dc,Dt = adknn_cobyla_diag.split(X[:int(0.25*len(Y)),:],Y[:int(0.25*len(Y))],T[:int(0.25*len(Y))])
Xtest = X[int(0.25*len(Y)):,:]
Xc,Yc,Tc = Dc
Xt,Yt,Tt = Dt
