...
def select_parameters(results, configs, stop_set, stop_criterion, choice_set, choice_criterion):
    ...
    for k in list(results['valid'].keys()):
        # To reduce dimension
    ...
    results_all = [dict([(k1, dict([(k2, v[i,]) for k2,v in results_sel[k1].items()]))
                        for k1 in list(results_sel.keys())]) for i in range(len(configs))]
...
def plot_evaluation_cont(results, configs, output_dir, data_train_path, data_test_path, filters=None):
    ...
    if filters is not None:
        ...
        I = [i for i in range(N) if np.all( \
                [configs[i][k]==filters[k] for k in list(filters.keys())] \
            )]

        results = dict([(s,dict([(k,results[s][k][I,]) for k in list(results[s].keys())])) for s in ['train', 'valid', 'test']])
...
def plot_evaluation_bin(results, configs, output_dir, data_train_path, data_test_path, filters=None):
    ...
    if filters is not None:
        ...
        N = len(configs)
        I = [i for i in range(N) if np.all( \
                [cmp(configs[i][k],filters[k]) for k in list(filters.keys())] \
            )]
    ...
    for eval_set in ['train', 'valid', 'test']:
        ...
        x = np.array(list(range(len(pc)))).astype(np.float32)/(len(pc)-1)
        for i in range(topk):
...
def plot_cfr_evaluation_bin(results, configs, output_dir):
    ...
    results_alphas = [dict([(k1, dict([(k2, v[i,]) for k2,v in results_sel[k1].items()]))
                        for k1 in list(results_sel.keys())]) for i in range(len(alphas))]
    di = configs[0]['n_in']
    do = configs[0]['n_out']
    ...
    ts = list(range(err_test.shape[2]))
    for i in range(len(alphas)):
