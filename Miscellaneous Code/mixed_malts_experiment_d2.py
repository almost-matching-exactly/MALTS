...
for i in range(0,1):
    for j in range(0,len(n_array)):
        ...
        num_cont_imp, num_disc_imp, num_cont_unimportant, num_disc_unimportant = 5, 10, 0, 20 
        ...
        #------------------------------------------------------------------------------------
        #PMALTS
        pmalts_model, L_pmalts = pmalts.PMALTS(X,Y,T,discrete_col=dcov)
        dcate_pmalts, t_hat_pmalts, ate_pmalts = pmalts.PMALTS_Test(Xtest,pmalts_model,discrete_col=dcov)
        ...
        L2 = np.eye(m + (len(dcov)) + (len(dcov)*(len(dcov)-1)//2) )
        adknn2 = dml.distance_metric_learning(m,dcov=dcov,diag=diag,L=L2,distance=dml.d2)
        Dc,Dt = adknn2.split(X,Y,T)
        Xc,Yc,Tc = Dc
        Xt,Yt,Tt = Dt
        optresult2 = adknn2.optimize(X,Y,T,numbatch=1)
        Lstar2 = adknn2.L
        fLC2 = lambda y: Lstar2
        dfC2 = pd.DataFrame(Lstar2)
        dcate_cobyla2, t_hat_cobyla2 = adknn2.CATE(Xtest,X,Y,T,fLC=fLC2,fLT=fLC2)
        t_true_cobyla = t_true
        print("L matrix", file=log)
        print(dfC2, file=log)
        
        #MALTS Non-Gradient Optimization
        L1 = np.eye(m + ( len(dcov) ) )
        adknn1 = dml.distance_metric_learning(m,dcov=dcov,diag=diag,L=L1,distance=dml.d)
        Dc,Dt = adknn1.split(X,Y,T)
        Xc,Yc,Tc = Dc
        Xt,Yt,Tt = Dt
        optresult = adknn1.optimize(X,Y,T,numbatch=1)
        Lstar1 = adknn1.L
        fLC1 = lambda y: Lstar1
        dfC1 = pd.DataFrame(Lstar1)
        dcate_cobyla1, t_hat_cobyla1 = adknn1.CATE(Xtest,X,Y,T,fLC=fLC1,fLT=fLC1)
        t_true_cobyla = t_true
        print("L matrix", file=log)
        print(dfC1, file=log)
        ...
        fig = figure(figsize=(10,10))
        rcParams.update({'font.size': 36})
        identity_line = np.linspace(min(min(t_true_cobyla), min(t_hat_cobyla2)), max(max(t_true_cobyla), max(t_hat_cobyla2)))
        plot(identity_line, identity_line, color="black", linestyle="dashed", linewidth=3.0)
        scatter(t_true_cobyla,t_hat_cobyla2,alpha=0.15)
        scatter(t_true_cobyla,t_hat_cobyla1,alpha=0.15)
        xlabel('True Treatment')
        ylabel('Predicted Treatment')
        tight_layout()
...
#        delt_pmalts = np.array(list(map(np.abs,np.array(t_hat_pmalts) - np.array(t_true_pmalts))))
        ...
        delt_cobyla2 = np.array(list(map(np.abs,np.array(t_hat_cobyla2) - np.array(t_true_cobyla))))
        delt_cobyla1 = np.array(list(map(np.abs,np.array(t_hat_cobyla1) - np.array(t_true_cobyla))))
...
##        ylim(top=200)       
        ...
        violinplot(delt_cobyla2,positions=[2])
        violinplot(delt_cobyla1,positions=[3])
...
##        violinplot(delt_cem,positions=[5])
        ...
        xticks([1,2,3,4,5,7,8,9],['PMALTS','MALTS-D2','MALTS-D1','Genmatch','NN Matching','CEM','BART','Causal Forest','Diff. of RF'],rotation=75)
