...
for i in range(0,len(p_imp_array)):
    for j in range(0,len(n_array)):
        ...
        n,m = X.shape
        dftest,_,_ = dg.data_generation_dense_endo(20000, num_cov_dense, num_covs_unimportant,rho=0.2)
        ...
        scatter(t_true_insane,t_hat_insane,alpha=0.15,s=10)
        ...
        dcate_cobyla, t_hat_cobyla = adknn.CATE(Xtest,Xtest,Ytest,Ttest,fLC=fLC,fLT=fLC)
        t_true_cobyla = np.array(t_true)
        t_hat_cobyla = np.array(t_hat_cobyla)
        print("L matrix", file=log)
        print(dfC, file=log)
        
        def diameterMatchGroup(tup,Lstar):
            #xi = tup[2]
        ...
        plot(identity_line, identity_line, color="black", linestyle="dashed", linewidth=3.0)
        scatter(t_true_cobyla,t_hat_cobyla, alpha=0.3,s=30)
        xlabel('True Treatment')
        ylabel('Predicted Treatment')
        tight_layout()
        ...
        identity_line = np.linspace(min(min(t_true_cobyla), min(t_hat_cobyla)), max(max(t_true_cobyla), max(t_hat_cobyla)))
        plot(identity_line, identity_line, color="black", linestyle="dashed", linewidth=3.0)
        scatter(t_true_cobyla,t_hat_cobyla, c=diam1, alpha=0.3,s=30)
        colorbar()
        xlabel('True Treatment')
        ylabel('Predicted Treatment')
        tight_layout()
        fig.savefig('Figures/CATE_distancemetriclearning_endo_2_MALTS_NonGradient_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
...
#        popt, pcov = curve_fit(func, diam, err)
        Tdict = {0: '#1f77b4', 1: '#ff7f0e'}
        fig = figure(figsize=(10,10))
        blue = mpatches.Patch(color='#1f77b4',alpha = 0.65)
        orange = mpatches.Patch(color='#ff7f0e',alpha = 0.65)
        for g in np.unique(Ttest):
            ix = np.where(Ttest == g)
            scatter(diam[ix], err[ix], c = Tdict[g], label = g, alpha=0.3,s = 50)
        legend([blue,orange],['control','treated'])
#        fig = figure(figsize=(10,10))
#        rcParams.update({'font.size': 36})
#        scatter(diam,err,c=Ttest,alpha=0.1)
#        legend(['0','1'])
#        plot(diam2, func(diam2, popt[0],popt[1],popt[2]), color="black", linestyle="dashed", linewidth=3.0)
        xlabel('1/Diameter of MG')
        ylabel('Absolute CATE Error')
        tight_layout()
        fig.savefig('Figures/CATE_distancemetriclearning_endo_MALTS_Err_Diam_1_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
        ...
        for g in np.unique(Ttest):
            ix = np.where(Ttest == g)
            scatter(t_true_cobyla[ix], t_hat_cobyla[ix], c = Tdict[g], label = g, alpha=0.15,s = 40)
        legend([blue,orange],['control','treated'])
#        fig = figure(figsize=(10,10))
#        rcParams.update({'font.size': 36})
#        scatter(diam,err,c=Ttest,alpha=0.1)
...
#        plot(diam2, func(diam2, popt[0],popt[1],popt[2]), color="black", linestyle="dashed", linewidth=3.0)
        ...
        ylabel('Predicted Treatment')
        tight_layout()
        fig.savefig('Figures/CATE_distancemetriclearning_endo_MALTS_NonGradient_TC_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
#        
        Tdict = {0: '#1f77b4', 1: '#ff7f0e'}
        fig = figure(figsize=(10,10))
        blue = mpatches.Patch(color='#1f77b4',alpha = 0.65)
        ...
        for g in np.unique(Ttest):
            ix = np.where(Ttest == g)
            scatter(Xtest[ix,0], Xtest[ix,1], c = Tdict[g], label = g, alpha=0.1,s = 20)
        legend([blue,orange],['control','treated'])
#        fig = figure(figsize=(10,10))
#        rcParams.update({'font.size': 36})
#        scatter(diam,err,c=Ttest,alpha=0.1)
...
#        plot(diam2, func(diam2, popt[0],popt[1],popt[2]), color="black", linestyle="dashed", linewidth=3.0)
        ...
        ylabel('X2')
        tight_layout()
        fig.savefig('Figures/DGP_X1_X2_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
#        
...
#        utils.install_packages(StrVector(packnames),repos = "http://cran.us.r-project.org")
        base = importr('base')
        ...
        for g in np.unique(Ttest):
            ix = np.where(Ttest == g)
            scatter(t_true_crf[ix], t_hat_crf[ix], c = Tdict[g], label = g, alpha=0.1,s = 5)
        legend([blue,orange],['control','treated'])
#        fig = figure(figsize=(10,10))
#        rcParams.update({'font.size': 36})
#        scatter(diam,err,c=Ttest,alpha=0.1)
...
#        plot(diam2, func(diam2, popt[0],popt[1],popt[2]), color="black", linestyle="dashed", linewidth=3.0)
        ...
        ylabel('Predicted Treatment')
        tight_layout()
        fig.savefig('Figures/CATE_distancemetriclearning_endo_crf_TC_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
#        
...
#        
        ...
        for g in np.unique(Ttest):
            ix = np.where(Ttest == g)
            scatter(t_true_bart[ix], t_hat_bart[ix], c = Tdict[g], label = g, alpha=0.1,s = 5)
        legend([blue,orange],['control','treated'])
#        fig = figure(figsize=(10,10))
#        rcParams.update({'font.size': 36})
#        scatter(diam,err,c=Ttest,alpha=0.1)
...
#        plot(diam2, func(diam2, popt[0],popt[1],popt[2]), color="black", linestyle="dashed", linewidth=3.0)
        ...
        ylabel('Predicted Treatment')
        tight_layout()
        fig.savefig('Figures/CATE_distancemetriclearning_endo_BART_TC_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
#        
...
#        fill_between(h_cem[1][:-1],h_cem[0],alpha=0.2)
        ...
        ylabel('Frequency')
        tight_layout() 
        fig.savefig('Figures/error_endo_cates_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cov_dense)+'_'+str(num_covs_unimportant)+'.jpg')
