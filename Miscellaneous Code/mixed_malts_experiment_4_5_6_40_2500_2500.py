...
for i in range(0,1):
    for j in range(0,len(n_array)):
        ...
        dftest,_,_,_,_ = dg.data_generation_dense_mixed(5000, 5000, num_cont_imp, num_disc_imp, num_cont_unimportant, num_disc_unimportant)
...
#        fig.savefig('Figures/CATE_mixed_distancemetriclearning_PMALTS_'+str(num_control)+'_'+str(num_treated)+'_'+str(num_cont_imp)+'_'+str(num_disc_imp)+'_'+str(num_cont_unimportant)+'_'+str(num_disc_unimportant)+'_'+str(currenttime)+'.jpg')
        ...
        Xc,Yc,Tc = Dc
        Xt,Yt,Tt = Dt
        optresult = adknn.optimize(X,Y,T,numbatch=50)
...
#        utils.install_packages(StrVector(packnames))
        ...
        r <- matchit(treated ~ X0+X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13+X14+X15+X16+X17+X18+X19+X20+X21+X22+X23+X24+X25+X26+X27+X28+X29+X30+X31+X32+X33+X34+X35+X36+X37+X38+X39+X40+X41+X42+X43+X44+X45+X46+X47+X48+X49+X50+X51+X52+X53+X54,
        ...
        x_hat_genmatch = np.array(genmatch.mtch).T[:,:55]
        t_cont_genmatch = np.dot((x_hat_genmatch[:,:len(treatment_eff_coef)]),treatment_eff_coef)
        t_disc_genmatch = np.dot((x_hat_genmatch[:,len(treatment_eff_coef):(len(treatment_eff_coef)+len(treatment_eff_coef_d))]),treatment_eff_coef_d)
        t_ord_sec_genmatch = np.sum(dg.construct_sec_order(x_hat_genmatch[:,:(num_cont_imp+num_disc_imp)]), axis=1)
        t_true_genmatch =  t_cont_genmatch + t_disc_genmatch + t_ord_sec_genmatch
        ...
        x_hat_nearestmatch = np.array(nearestmatch.mtch).T[:,:55]
        t_cont_nearest = np.dot((x_hat_nearestmatch[:,:len(treatment_eff_coef)]),treatment_eff_coef)
        t_disc_nearest = np.dot((x_hat_nearestmatch[:,len(treatment_eff_coef):(len(treatment_eff_coef)+len(treatment_eff_coef_d))]),treatment_eff_coef_d)
        t_ord_sec_nearest = np.sum(dg.construct_sec_order(x_hat_nearestmatch[:,:(num_cont_imp+num_disc_imp)]), axis=1)
        t_true_nearestmatch =  t_cont_nearest + t_disc_nearest + t_ord_sec_nearest
...
#        x_hat_cem = np.array(cem.mtch).T[:,:55]
#        t_cont_cem = np.dot((x_hat_cem[:,:len(treatment_eff_coef)]),treatment_eff_coef)
#        t_disc_cem = np.dot((x_hat_cem[:,len(treatment_eff_coef):(len(treatment_eff_coef)+len(treatment_eff_coef_d))]),treatment_eff_coef_d)
#        t_ord_sec_cem = np.sum(dg.construct_sec_order(x_hat_cem[:,:(num_cont_imp+num_disc_imp)]), axis=1)
#        t_true_cem =  t_cont_cem + t_disc_cem + t_ord_sec_cem
#        
#        fig = figure(figsize=(10,10))
#        rcParams.update({'font.size': 36})
...
#        violinplot(delt_pmalts,positions=[1])
        ...
        violinplot(delt_bart,positions=[6])
        violinplot(delt_crf,positions=[7])
        violinplot(delt_insane,positions=[8])
        xticks([2,3,4,6,7,8],['MALTS','Genmatch','NN Matching','BART','Causal Forest','Diff. of RF'],rotation=75)
