...
class malts:
    ...
    def loss_(self, Mc, Md, xc1, xd1, y1, xc2, xd2, y2, gamma=1, reg_cons=0.00005 ):
        w12 = np.exp( -1 * gamma * self.distance(Mc,Md,xc1,xd1,xc2,xd2) )
        reg = np.exp( gamma * self.distance(Mc,Md,np.zeros_like(xc1),np.zeros_like(xd1),np.ones_like(xc2),np.ones_like(xd2) ) )
        return w12*((y1-y2)**2) + reg_cons*reg
