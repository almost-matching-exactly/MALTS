...
np.random.seed(0)
diff_mean = []
overlaps = np.sqrt([400,25,1,0.1,0.0001])
...
for ni in range(len(ns)):
    ...
    for i in range(len(overlaps)):
        n = ns[ni]
        overlap = overlaps[i]
        df_data, df_true, discrete = dg.data_generation_dense_mixed_endo(n, p, 0, 3*p, 0, rho=0, scale=1, overlap=overlap)
        # axi = axes[i]
        ...
        err['Overlap (Standardized Difference of Means)'] = [diff_mean[i] for a in range(len(label_malts + label_bart + label_crf + label_genmatch + label_psnn + label_full + label_prog))]
        err['Overlap'] = ['%.3f\n ($\\epsilon_t$ = %.2f)'%(diff_mean[i],overlap) for a in range(len(label_malts + label_bart + label_crf + label_genmatch + label_psnn + label_full + label_prog))]
    ...
    if ni==0:
        sns.lineplot(hue='Method',y='Error',x='Overlap (Standardized Difference of Means)', data=df_err,style='Method',markers=True,linewidth=3.5,markersize=10, ax=axi)
        axi.legend(bbox_to_anchor=(1., 1.05))
    else:
        sns.lineplot(hue='Method',y='Error',x='Overlap (Standardized Difference of Means)', data=df_err,style='Method',markers=True,linewidth=3.5,markersize=10, ax=axi,legend=False)
...
fig.savefig('Figures/trend_multifold_malts_overlap.png')
